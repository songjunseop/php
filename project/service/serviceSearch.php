...
<head>
    ...
    <title>문의글 검색</title>
            ...
            <!-- 주소
        ekfvoddl0321.dothome.co.kr/project/board/serviceSearch.php 
...
<body>
    ...
    $sql = "SELECT b.serviceID, b.serviceTitle, b.serviceContents, m.youName, b.regTime, b.serviceView FROM myProject_Service b JOIN myProject m ON(b.memberID = m.memberID) WHERE b.serviceTitle LIKE '%{$searchKeyword}%' ORDER BY serviceID DESC" ;
    // switch($searchOption){
    //     case 'title':
    //         $sql .= "WHERE b.serviceTitle LIKE '%{$searchKeyword}%' ORDER BY boardID DESC";
    //         break;
    //     case 'content':
...
?>
    ...
    //b.boardID, b.boardTitle, m.youName, b.regTime, b.boardView
        ...
        // switch($searchOption){
        //     case 'title':
        //         $sql .= "WHERE b.serviceTitle LIKE '%{$searchKeyword}%' ORDER BY boardID DESC LIMIT {$pageLimit}, {$pageView}";
        //         break;
        //     case 'content':
        ...
        if($result){
            ...
            if($result > 0){
                for($i=1; $i<=$count; $i++){
                    ...
                    echo "<tr>";
                    echo "<td class='blueNumber'>".$serviceInfo['serviceID']."</td>";
...
?>
                ...
                <div class="right">
                    <a href="service.php" class="search-btn">목록보기</a>
                </div>
    ...
    if($endPage >= $serviceCount){
        $endPage = $serviceCount;
    }
    // echo $boardCount;
